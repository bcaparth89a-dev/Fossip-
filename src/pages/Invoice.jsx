...
// Phonne number logic -----
 ...
 const handlePhone = (event) => {
  ...
  <button onClick={() => setdisplay(!display)} className="flex-shrink-0">
    <img
      src={id}
      alt="User"
  ...
  <p className="font-bold text-base sm:text-lg md:text-xl lg:text-xl">Invoice Preview</p>
</div>
...
<div className="px-4 mt-4 flex flex-col gap-1.5">
  ...
  {products.map((product, index) => (
    <tr key={index} className="border-b border-gray-200">
      ...
      <td className="pl-2">{product.qty * product.unitPrice}</td>
    </tr>
  ))}
</tbody>
                ...
                <table className="w-full text-[16px]">
                  <tbody>
                    <tr>
                      ...
                      <td className="p-2 text-right whitespace-nowrap">₹ {subtotal}</td>
                    </tr>
                    <tr>
                      <th className="p-2 text-right w-1/2">Discount</th>
                      <td className="p-2 text-right whitespace-nowrap">₹ {discount}</td>
                    </tr>
                    <tr>
                      <th className="p-2 text-right w-1/2">Total</th>
                      <td className="p-2 text-right whitespace-nowrap">₹ {subtotal - discount}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
  ...
  {products.map((product) => (
    <div key={product.id} className="mb-2">
      ...
      <div className="flex justify-between items-center gap-2">
        <div className="flex items-center bg-white px-4 rounded-lg shadow-md h-7 
    ...
    {/* part 2 */}
<div className="w-full mt-8 px-3 sm:px-4 md:px-6">
...
<div className="flex justify-center sm:justify-end gap-3 mt-6 flex-wrap">
  <button
    onClick={handleSave}
    className="h-8 px-3 sm:px-4 text-xs sm:text-sm bg-blue-500 text-white text-center border-2 border-blue-600 rounded hover:bg-white hover:text-black transition"
  >
    Submit
  </button>
  <button
